...
function initializePlannerFrames(build) {
  ...
  setPlannerFrameSrc(document.querySelector("#proficiencyPlannerFrame"), plannerUrl("/weapon-proficiency.html", { embed: "damage", v: "20260913-1", vocation: build.state.stats.vocation, build: build.state.proficiencyPlanner.token }));
}
...
function hydrateInactiveBuild(build) {
  ...
  if (build.state.proficiencyPlanner.token && !build.state.proficiencyPlanner.effects.length) {
    ...
    document.querySelector("#proficiencyHydrateFrame").src = plannerUrl("/weapon-proficiency.html", { embed: "damage", v: "20260913-1", vocation: build.state.stats.vocation, build: build.state.proficiencyPlanner.token });
  }
}
