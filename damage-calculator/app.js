...
function hydrateInactiveBuild(build) {
  ...
  if (build.state.proficiencyPlanner.token && !build.state.proficiencyPlanner.effects.length) {
    ...
    document.querySelector("#proficiencyHydrateFrame").src = plannerUrl("/weapon-proficiency.html", { embed: "damage", v: "20260906-2", vocation: build.state.stats.vocation, build: build.state.proficiencyPlanner.token });
  }
}
