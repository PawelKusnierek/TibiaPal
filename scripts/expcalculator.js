function submit_form() {
  ...
  if (typeof total_experience_needed === 'string') {
    ...
    if (exp_per_hour > 0) {
      ...
      let time_to_target_level = "At " + exp_per_hour / 1000000 + " kk/h, time needed to reach target level: " + Math.floor(days_needed) + " day(s), " + Math.floor(hours_needed) + " hour(s), " + Math.floor(minutes_needed) + " minute(s) and " + Math.round(seconds_needed) + " second(s).";
      if (hunt_per_day > 0) {
